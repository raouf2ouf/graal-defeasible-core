...
public class DLGP2Parser implements DLGP2ParserConstants {
  ...
  Object ns;
    ...
    if (prefixManager.containsPrefix(prefix))
    {
      {if (true) throw this.generateParseException("Prefix already declared: " + prefix);}
    }
  ...
  Token t;
    ...
    if (isBaseDeclared)
    {
      {if (true) throw this.generateParseException("Base already declared.");}
    }
    isBaseDeclared = true;
...
// Preference
  final public ParseEvent preference() throws ParseException {
  ArrayList < Object > listTerm = new ArrayList < Object > ();
    term(listTerm);
    if (jj_2_62(2)) {
      jj_consume_token(64);
      term(listTerm);
    {if (true) return new FindsPreferenceEvent(listTerm.get(0), listTerm.get(1), ParserListener.OBJECT_TYPE.RULE_PREFERENCE);}
    } else if (jj_2_63(2)) {
      jj_consume_token(65);
      term(listTerm);
        {if (true) return new FindsPreferenceEvent(listTerm.get(0), listTerm.get(1), ParserListener.OBJECT_TYPE.ALTERNATIVE_PREFERENCE);}
    } else {
      jj_consume_token(-1);
      throw new ParseException();
    }
